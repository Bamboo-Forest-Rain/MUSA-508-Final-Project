...
```{r combine census info with fishnet}
...
```{r Call by Fishnet Time Separated}
DAT_map <-               
fishnet_all %>% 
            mutate(weekend = ifelse(dotw %in% c("Sun", "Sat"), "Weekend",         
                                    "Weekday"),
                time_of_day = case_when(
                                 hour(interval60) >= 7 & hour(interval60) < 12 ~  
                                "Morning to noon",
                                 hour(interval60) >= 12 & hour(interval60) < 17 ~ 
                                "Afternoon",
                                 hour(interval60) >= 17 & hour(interval60) < 22 ~
                                "Evening",
                                hour(interval60) >= 22 & hour(interval60) <= 6 ~
                                "Overnight")) %>%
...
ggplot()+
  geom_sf(data = DAT_map,
            aes(fill = q5(n))) +
  scale_fill_brewer(palette = "BuPu",direction = 1,label = c(0,))+
  facet_grid(weekend ~ time_of_day)+
  labs(title="EMS call per hr by fishnet. March to September, 2021")+
  mapTheme()
```
...
fishnet_all <-
  fishnet_all %>% 
mutate(school_dist= st_distance(st_coid(fishnet_all),st_coid(school.sf)))
```
...
fishnet_all <-
  fishnet_all %>%
  group_by(uniqueID)%>%
  mutate(countCrime2 = sum(countCrime))%>%
  ungroup
```
