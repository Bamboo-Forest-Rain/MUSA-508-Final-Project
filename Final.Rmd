...
Tracing this logic, we gathered population-related data from the American Community Survey by the U.S. Census Bureau. The data is avaliable on a census tract level. We filtered out the census tracts that belong to Santa Monica.  

```{r Census with Age}
#Santa Monica Census Information
SanMon_Census <- 
...
We examined the call demand by day of the week and by hour to see if the time process matters. Fig 1 plots the number of EMS calls against hour of the day. Each line on the figure represents a day in a week. 
...
Injuries are often caused by aggressive actions, and these aggressive actions are outcomes of violation of laws. We downloaded the crime data from the city's open data portal and filtered out crime that are related to violence, firearms, murder, and aggressive assault. 
...
We downloaded the weather data measured by Santa Monica Airport. We will access its correlation with demand later. 
...
correlation.long <-
  st_drop_geometry(fishnet_all) %>%
    dplyr::select(-uniqueID, -cvID, -dotw, -weekend, -time_of_day,-week,-Temperature,-interval60) %>%
...
ggplot(correlation.long, aes(Value, countEMS)) +
  ...
  labs(title = "EMS count as a function of Interested Variables") +
  plotTheme()
```
...
```{r}
# Histogram for every variable, inclufing y
correlation.long <-
  st_drop_geometry(fishnet_all) %>%
    dplyr::select(-uniqueID, -cvID, -dotw, -weekend, -time_of_day,-week,-Temperature) %>%
    gather(Variable, Value, -countEMS)
...
error.fold.space[is.na(error.fold.space)] <- 0
 ...
 error.fold.space %>%
  ggplot(aes(MAE)) + 
    ...
    labs(title="Distribution of MAE", subtitle = "LOGO cross validation: Per Fishnet",
...
error.fold.space %>%
  ...
  geom_sf(data = fishnet) +
    ...
    labs(title = "EMS count errors by LOGO Cross-validation by time") +
    mapTheme() + theme(legend.position="bottom",
