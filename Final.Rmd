...
```{r read}
Fire_call <- read.socrata("https://data.smgov.net/resource/5y3u-5db4.json")
...
Sept_Dec19 <- Fire_call2 %>%
  filter(year == "2021" & month == c("9","10","11"))

Sept_Dec19EMS <- Sept_Dec19 %>%
  filter(call_type_description == "Emergency Medical Service (EMS)")%>%
  mutate(response_time = as.numeric(cleared - received))
```
