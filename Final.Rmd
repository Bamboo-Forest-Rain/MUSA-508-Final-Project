...
ggplot(Fire_call %>% mutate(hour = hour(received)))+
  ...
  labs(title="EMS Calls by Day of the Week March to November",
       x="Hour", 
       y="EMS Call Counts")+
     plotTheme()
```
...
ggplot(Fire_call %>% 
  ...
  labs(title="EMS call - weekend vs weekday, March\n and November, 2021",
       x="Hour", 
       y="EMS Call Count", caption = "Fig 5")+
     plotTheme()
...
=======
  select(geometry, ucr)
>>>>>>> 2c2b6b3f40a4794f9577c04cd2b9306594c3eba6
...
fishnet_all <-
  fishnet_all %>% 
mutate(school_dist= as.numeric(st_distance(st_coid(fishnet_all),school.cen)))
```
...
week_predictions <- 
  ride.Test.weekNest %>% 
    mutate(OnlySocial = map(.x = data, fit = reg1, .f = model_pred),
          SocialwithGeo = map(.x = data, fit = reg2, .f = model_pred))%>%     gather(Regression, Prediction, -data, -week) %>%
...
error.fold.space[is.na(error.fold.space)] <- 0
 ...
 error.fold.space %>%
  ggplot(aes(MAE)) + 
    ...
    labs(title="Distribution of MAE", subtitle = "k-fold cross validation: Per Fishnet",
 ...
 error.fold.time <- 
  ...
  ggplot(aes(MAE)) + 
    geom_histogram(bins = 30, colour="black", fill = "#FDE725FF") +
    geom_vline(xintercept = 0) + scale_x_continuous(breaks = seq(0, 14, by = 2)) + labs(title="Distribution of MAE", subtitle = "k-fold cross validation: Per week",
         x="Mean Absolute Error", y="Count") +
    plotTheme()
