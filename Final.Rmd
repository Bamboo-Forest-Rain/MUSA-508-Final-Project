...
SanMonTracts <- 
  SanMon_Census %>%
  as.data.frame() %>%
  distinct(GEOID, .keest_crsp_all = TRUE) %>%
...
fishnet <-st_transform(fishnet, crs = 4326)
```
...
fishnet_all <-
  st_join(fishnet_all, SanMon_Crime)
...
#See Pickupby week
group_by(ride.Train, week, uniqueID) %>%
  ...
  ggplot() + geom_sf(data = SanMon_Bound, fill="grey50") + geom_sf(aes(fill = countEMS)) +
    ...
    labs(title="Sum of calls by fishnet by week") +
