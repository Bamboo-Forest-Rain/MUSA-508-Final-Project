...
Santa Monica has one of the country's most inefficient emergent medical service (EMS) system. The national average time for an ambulance to respond a emergency call is 7 minutes. Even in a rural setting, the average response time is 14 minutes. Santa Monica, however, based on the city data from March to November 2021, takes averagely 30 minutes from receiving the call to arrive the destination, much longer than that of the national average. The response time over different locations in the city is displayed in the map below. 
...
knitr::include_graphics("santa-monica.png")
```
...
ggplot(correlation.long_all, aes(Value)) + 
    ...
    label(title = "Distribution of Variables", caption = "Fig 7")
  plotTheme()
...
stargazer(reg1, out = "html")
stargazer(reg2, out = "html")
```
...
```{r}
#Hanpu check if you have anything to do with this repeated chunk
