...
fishnet_all %>%
    dplyr::select(countEMS, time_of_day) %>%
      ggplot(., aes(x=time_of_day,y=countEMS)) +   
        geom_bar(position = "dodge", stat="identity") +
        labs(x="Time of the Day", y="countEMS",
             title = "EMS count associated with time of the day",
...
reg2 <- 
  lm(countEMS ~  hour(interval60) + Age_above60 + Age_below10 + Med_Inc + countCrime + Temperature + DowntownDistance + bars.nn + school.nn + dotw + time_of_day, data=ride.Train)
...
week_predictions <- 
  ride.Test.weekNest %>% 
    mutate(OnlySocial = map(.x = data, fit = reg1, .f = model_pred),
           SocialwithDistance = map(.x = data, fit = reg2, .f = model_pred))%>%     gather(Regression, Prediction, -data, -week) %>%
