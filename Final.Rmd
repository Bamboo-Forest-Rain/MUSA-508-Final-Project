...
fishnet <- 
  st_make_grid(SanMon_Bound, cellsize = 100) %>%
  st_sf() %>%
...
ggplot() + 
  ...
  stat_density2d(data = data.frame(st_coordinates(Fire_call_geo)), 
...
ggplot() + 
  geom_sf(data = Boundary, fill = "white", color = "grey40") +
  stat_density2d(data = data.frame(st_coordinates(Fire_call_geo)), 
                 aes(X, Y, fill = ..level.., alpha = ..level..),
                 size = 0.01, bins = 40, geom = 'polygon') +
  scale_fill_viridis() +
